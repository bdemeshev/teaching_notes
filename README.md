...
Соображения про quatro-qmd:
* удобно для коротких онлайн заметок;
* вряд ли обыграет чистых латех для большой pdf-книжки;
* knitr (?) осуществляет подмену всех newcommand, DeclareMathOperator в блоке .hidden-content в начале qmd перед компиляцией html, tex. 
При таком определении макрокоманд видны формулы в локальном html и в html на гите. 
Выходит ужасный тех-файл с распарсенными newcommand, но для онлайн похоже так лучше. 
* Можно поставить отдельные блоки для tex и html, но тогда формулы видны только в локальном html, а в html с гита --- не видны.
...
* Нравится, что в американских школах рассказывают не только про график параболы и гиперболы, но и про график логистической кривой.
...
* Дорогие преподаватели теории вероятностей, лучше делать пункты вопросов независимыми.
Например, типичный вопрос про случайную величину $X$ с функцией плотности выглядит так:
У случайной величины $X$ функция плотности равна $f(x) = cx$ на отрезке $[0;2]$.
а) Найдите константу $c$.
б) Найдите ожидание величины $X$. 
Так вполне можно спросить на семинаре, но не надо так ставить вопрос на контрольной работе. 
Никогда-никогда в первом пункте задачи про функцию плотности не просите найти нормировочную константу. 
Во-первых, это подвязывает остальные пункты на этот пункт. И студент, знающий, как решить пункты дальше,
не берётся за них из-за того, что не решил первый пункт. 
Во-вторых, подавляющее большинство слабых студентов знает, что интеграл под плотностью равен 1, 
и не может решить этот пункт только потому, что ошибается в интеграле. 
В итоге ноль за задачу при подобном подходе может получить человек с хорошей вероятностной интуицией, не умеющий интегрировать. 
Как спросить про нормировочную константу по-другому?
Сделайте вопрос независимым от других пунктов и поставьте его в конец!
Например, 
У случайной величины $X$ функция плотности равна $f(x) = 0.5x$ на отрезке $[0;2]$.
а) Найдите ожидание величины $X$. 
б) Найдите константу $c$, если $g(x) = cxf(x)$ — это тоже некая функция плотности. 
или 
б') Без вычислений найдите интеграл $\int_0^2 100 f(x) dx$. 

#probability

* Дорогие преподаватели теории вероятностей :) 
Хватит мучить студентов таблицами. 
Есть приложения для iOS, android и веб-странички. 
Например, https://homepage.divms.uiowa.edu/~mbognar/.
Гораздо нагляднее получается!

Ещё можно в стандартную табличку нормальной функции распределения добавить всего несколько фактов:
$F(-t) = 1 - F(t)$, $P(a<=X<=b) = F(b) - F(a)$, $F(\infty) = 1$, $F(-\infty) = 0$.

Сразу гораздо большее количество слабых студентов смогут ею воспользоваться. 
Если уж очень хочется, можно объявить, что этих формул не будет в таблицах на экзамене, 
но они будут в таблицах на семинарах. 

#probability

* Не стоит загромождать идейную задачу большим объемом вычислений. 
Чем меньше вычислений, чем виднее идея, тем лучше!
Часто в задаче есть совершенно ненужный параметр. 
Что-нибудь в духе "В круге радиуса $R$ выбирается точка, 
найдите функцию плотности расстояния до центра круга". 
Можно обойтись без $R$.
А если уж $R$ очень нужен по сюжету дальше, то начать вопросы с частного случая. 

* Дайте шанс слабым студентам что-то решить!
Например, первым пунктом задачи может идти частный случай второго, более общего пункта. 

* Стоит разнообразить персонажей задач.
Можно воскрешать давно забытых героев прошлого. 
Например, Кржемелик и Вахмурка.

https://ru.wikipedia.org/wiki/%D0%9A%D1%80%D0%B6%D0%B5%D0%BC%D0%B5%D0%BB%D0%B8%D0%BA_%D0%B8_%D0%92%D0%B0%D1%85%D0%BC%D1%83%D1%80%D0%BA%D0%B0

* Добавляйте ссылки на хорошие видео в курс!

Здесь очень нужна подборка хороших вероятностных анимаций с описаниями!

https://www.youtube.com/playlist?list=PLGpdGKp2JUvwmeJDqXpFP4JbXbvAFIL9-

* Говорящие название для операторов!

Лучше span вместо L, лучше rank вместо rk.
Лучше Var вместо V, лучше row_2 A вместо A_2.

* К тексту можно добавить не только кликабельные ссылки, но и qr-коды.
Если текст распечатают, то ссылки перестанут быть кликабельны, 
а qr-коды останутся удобными!
